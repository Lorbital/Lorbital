...
(function () {
  ...
  var zh = {
    ...
    explorer: {
      // Category selector
      introTitle: '量子轨道探测器',
      introText: '本页提供氢原子轨道的 3D 点云可视化。氢原子是最简单的原子系统，其薛定谔方程可以精确求解，从而得到电子在不同量子态下的轨道波函数。这些模型通过求解氢原子的薛定谔方程计算得出，使用 Python 通过蒙特卡洛拒绝采样法生成，根据波函数的概率密度分布 |ψ|² 进行采样，使得点云的密度准确反映电子在原子核周围不同位置出现的概率。页面同时加入了正十二面体与正二十面体代表分子的 DFT 结果：先从理想笼架出发进行几何优化，再将总电子密度与 HOMO 导出为可叠加查看的点云。请从下方入口进入周期表、g 轨道或分子笼模型。',
